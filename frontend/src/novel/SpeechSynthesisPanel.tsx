...
export function SpeechSynthesisPanel({novelId,characterId,onInspect}:{novelId?:string;characterId?:string;onInspect?:(inspection:AudioInspection)=>void}){
  ...
  useEffect(()=>{api.audioProviders().then(data=>setProviders((data.items||[]).filter(item=>item.capabilities.includes('TTS')&&item.configured))).catch(()=>setProviders([]))},[]);
  ...
  return <Panel title="声音合成" className="speech-production">
    ...
    <Button loading={loading} disabled={!text.trim()||!providerId.trim()||(providerId!=='auto'&&!modelId.trim())} onClick={synthesize}>{task?.status==='FAILED'?'重新生成':'生成语音'}</Button>
