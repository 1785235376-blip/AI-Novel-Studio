...
describe('ChapterTree',()=>{
  ...
  it('permanently deletes only from the archived chapter list after confirmation',async()=>{const onDelete=vi.fn();render(<ChapterTree chapters={[{id:'c1',title:'正文章'}]} archived={[{id:'c2',title:'已移出章'}]} onSelect={vi.fn()} onCreate={vi.fn()} onRestore={vi.fn()} onDelete={onDelete}/>);expect(screen.queryByRole('button',{name:'永久删除章节正文章'})).toBeNull();fireEvent.click(screen.getByRole('button',{name:'永久删除章节已移出章'}));expect(screen.getByRole('alertdialog').textContent).toContain('无法撤销');fireEvent.click(screen.getByRole('button',{name:'永久删除章节'}));await waitFor(()=>expect(onDelete).toHaveBeenCalledWith(expect.objectContaining({id:'c2'})))});
});
