...
describe("MultimodalDirectorWorkspace", () => {
  beforeEach(() => {
    ...
    vi.stubGlobal("URL", {
      ...URL,
      createObjectURL: vi.fn(() => "blob:asset-preview"),
  ...
  it("adds a semantic image reference and emits constraints", () => {
    ...
    render(
      <MultimodalDirectorWorkspace
        mode="image"
        onConstraintsChange={onChange}
      />,
    );
    fireEvent.change(screen.getByLabelText("参考语义"), {
      target: { value: "构图" },
    });
    fireEvent.change(screen.getByPlaceholderText("粘贴本地或已上传图片地址"), {
      target: { value: "ref://hero" },
    });
    ...
    expect(onChange).toHaveBeenLastCalledWith(
      expect.objectContaining({
        references: [
          expect.objectContaining({ role: "构图", uri: "ref://hero" }),
        ],
      }),
    );
  });
  ...
  it("adds a director shot in video mode", () => {
    ...
    expect(onShotsChange).toHaveBeenLastCalledWith(
      expect.arrayContaining([expect.objectContaining({ name: "镜头 01" })]),
    );
  });
  ...
  it("emits updated prop asset constraints", async () => {
    ...
    expect(onChange).toHaveBeenLastCalledWith(
      expect.objectContaining({
        consistency: expect.objectContaining({ prop_asset_ids: ["prop-1"] }),
      }),
    );
  });
  it("adds a project asset with its scoped preview and metadata", async () => {
    render(<MultimodalDirectorWorkspace mode="image" novelId="novel-assets" />);
    fireEvent.click(await screen.findByRole("button", { name: "怀表.png" }));
    ...
    expect(node.textContent).toContain("image/png");
  });
  ...
  it("emits dialogue and subtitle fields on the director shot", () => {
    ...
    fireEvent.change(screen.getByLabelText("镜头对白"), {
      target: { value: "快走！" },
    });
    fireEvent.change(screen.getByLabelText("镜头字幕"), {
      target: { value: "快走！" },
    });
    expect(onShotsChange).toHaveBeenLastCalledWith(
      expect.arrayContaining([
        expect.objectContaining({ dialogue: "快走！", subtitle: "快走！" }),
      ]),
    );
  });
  ...
  it("opens the advanced shot cards with shared shot state", () => {
    ...
    fireEvent.change(screen.getAllByLabelText("镜头名称").at(-1)!, {
      target: { value: "高级镜头" },
    });
    expect(onShotsChange).toHaveBeenLastCalledWith(
      expect.arrayContaining([expect.objectContaining({ name: "高级镜头" })]),
    );
  });
  ...
  it("adds semantic references to the local binding summary", () => {
    render(
      <MultimodalDirectorWorkspace mode="image" novelId="bindings-test" />,
    );
    fireEvent.change(screen.getByPlaceholderText("粘贴本地或已上传图片地址"), {
      target: { value: "ref://character" },
    });
    fireEvent.click(screen.getByText("添加"));
  ...
  it("restores binding manifest entries when importing a confirmed constraint package", async () => {
    render(
      <MultimodalDirectorWorkspace mode="image" novelId="import-bindings" />,
    );
    const file = new File(
      [
        JSON.stringify({
          mode: "image",
          references: [
            { uri: "ref://imported", role: "角色", position: { x: 20, y: 30 } },
          ],
          bindings: {
            characters: ["导入角色"],
            scenes: ["导入场景"],
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/绑定清单：角色 2 · 场景 1 · 道具 1/),
      ).toBeTruthy(),
    );
  ...
  it("does not overwrite bindings when an invalid package is selected", async () => {
    render(
      <MultimodalDirectorWorkspace mode="image" novelId="invalid-bindings" />,
    );
    fireEvent.change(screen.getByPlaceholderText("粘贴本地或已上传图片地址"), {
      target: { value: "ref://existing" },
    });
    fireEvent.click(screen.getByText("添加"));
    const file = new File(
      [JSON.stringify({ bindings: { characters: "not-an-array" } })],
      "invalid.json",
      { type: "application/json" },
    );
    fireEvent.change(screen.getByLabelText("导入 JSON"), {
    ...
    await waitFor(() =>
      expect(screen.getByText("约束包格式无效")).toBeTruthy(),
    );
    expect(screen.getByText(/绑定清单：角色 1/)).toBeTruthy();
  });
  ...
  it("saves, restores, and deletes a consistency profile", () => {
    ...
    fireEvent.change(screen.getByLabelText("角色外观"), {
      target: { value: "黑色短发" },
    });
    ...
    fireEvent.change(screen.getByLabelText("一致性档案"), {
      target: { value: "主角标准造型" },
    });
    expect((screen.getByLabelText("角色外观") as HTMLInputElement).value).toBe(
      "黑色短发",
    );
