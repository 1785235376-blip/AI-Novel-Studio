...
export function ModelCenter(){
 ...
 const refresh=async()=>{const generation=++requestGeneration.current;if(mounted.current){setError('');setLoading(true)}try{const [m,r,p]=await Promise.all([api.modelCenterModels(),api.modelCenterRuntimes(),api.modelCenterPipelines()]);if(mounted.current&&generation===requestGeneration.current){setModels(m.items);setRuntimes(r.items);setPipelines(p.items)}}catch{if(mounted.current&&generation===requestGeneration.current)setError('模型中心状态读取失败，请检查本地服务。')}finally{if(mounted.current&&generation===requestGeneration.current)setLoading(false)}};
 useEffect(()=>{mounted.current=true;void refresh();return()=>{mounted.current=false;requestGeneration.current++}},[]);
 const action=async(id:string,kind:'validate'|'start'|'stop')=>{setBusy(`${id}:${kind}`);setError('');try{if(kind==='validate')await api.modelCenterValidateRuntime(id);else if(kind==='start')await api.modelCenterStartRuntime(id);else await api.modelCenterStopRuntime(id);if(mounted.current)await refresh()}catch{if(mounted.current)setError('运行时操作未完成。请确认路径、localhost 绑定和运行时状态。')}finally{if(mounted.current)setBusy('')}};
