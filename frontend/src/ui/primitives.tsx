...
export function Panel({title,actions,children,className=''}:{title?:ReactNode;actions?:ReactNode;children:ReactNode;className?:string}){return <section className={`ui-panel ${className}`.trim()}>{(title||actions)&&<header><h2>{title}</h2>{actions}</header>}{children}</section>}
