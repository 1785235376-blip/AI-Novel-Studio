...
const moduleLabels: Record<StudioModule, string> = { NOVEL: '小说', IMAGE: '图片', VIDEO: '视频', AUDIO: '声音', PLUGIN: '插件', WORKFLOW: '工作流', ASSETS: '资产' };
...
export function ModuleWorkspaceRoutes({module,onModuleChange,scope,actor,novelId}:Props){
  ...
  switch(module){
    case 'IMAGE': main=<><VisualContextPanel novelId={novelId ?? ''}/><VisionAnalysisPanel novelId={novelId ?? ''}/><ImageGenerationPanel novelId={novelId ?? ''} onInspect={setImageInspection}/></>; status='图片工作区'; break;
    case 'VIDEO': main=novelId?<ScreenplayPanel novelId={novelId} onInspect={setVideoInspection}/>:<p className="novel-help">请先打开小说项目，再进入视频工作区。</p>; status='视频工作区'; break;
    ...
    case 'PLUGIN': main=<><AiControlCenter/><PluginManagerPanel onInspect={setPluginInspection}/></>; status='控制中心与插件'; break;
  ...
  return <AppShell module={module} onModuleChange={onModuleChange} scope={scope} actor={actor} sidebar={<WorkspaceRail module={module}/>} main={<><CapabilityStatusCenter module={module as any} novelId={novelId}/>{main}</>} inspector={inspector} status={status}/>;
}
