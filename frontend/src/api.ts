...
export type CredentialStatus={provider:string;configured:boolean;backend:string;secret:null};
...
export const api={
 ...
 assetDownloadUrl:(assetId:string)=>`/api/assets/${encodeURIComponent(assetId)}/download`,
 assetDownload:(assetId:string)=>downloadCall(`/api/assets/${encodeURIComponent(assetId)}/download`),
 ...
 worldRules:(novelId:string,status?:string)=>call<{items:any[];storage:string}>(query(`/api/novels/${encodeURIComponent(novelId)}/world-rules`,status?{status}:{})),
   ...
   configureVideoProvider:(providerId:string,config:{endpoint:string;model_id:string;enabled:boolean})=>call<any>(`/api/video-providers/${encodeURIComponent(providerId)}/config`,{method:'PUT',body:JSON.stringify(config)}),
