...
export const api={
 ...
 worldRules:(novelId:string,status?:string)=>call<{items:any[];storage:string}>(query(`/api/novels/${encodeURIComponent(novelId)}/world-rules`,status?{status}:{})),
   ...
   audioProductionSettings:(novelId:string)=>call<{novel_id:string;voice_bindings:Array<{character_id:string;provider_id:string;model_id:string;voice:string;emotion:string}>;pronunciation_dictionary:Array<{term:string;pronunciation:string}>}>(`/api/novels/${encodeURIComponent(novelId)}/audio-production/settings`),
   updateAudioProductionSettings:(novelId:string,body:{voice_bindings:Array<{character_id:string;provider_id:string;model_id:string;voice:string;emotion:string}>;pronunciation_dictionary:Array<{term:string;pronunciation:string}>})=>call<any>(`/api/novels/${encodeURIComponent(novelId)}/audio-production/settings`,{method:'PUT',body:JSON.stringify(body)}),
   queueAudiobookChapter:(novelId:string,chapterId:string,config:{provider_id:string;model_id:string;voice:string;emotion?:string;character_id?:string;speech_rate?:number;pause_ms?:number}={provider_id:'openai',model_id:'gpt-4o-mini-tts',voice:'alloy'})=>call<any>(`/api/novels/${encodeURIComponent(novelId)}/audiobook/chapters/${encodeURIComponent(chapterId)}/queue`,{method:'POST',body:JSON.stringify(config)}),
   ...
   cancelAudiobookJob:(novelId:string,jobId:string)=>call<any>(`/api/novels/${encodeURIComponent(novelId)}/audiobook/jobs/${encodeURIComponent(jobId)}/cancel`,{method:'POST'}),
