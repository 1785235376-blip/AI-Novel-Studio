...
const tokens=readFileSync(new URL('../src/ui/tokens.css',import.meta.url),'utf8');const ui=readFileSync(new URL('../src/ui/ui.css',import.meta.url),'utf8');const novel=readFileSync(new URL('../src/novel/novel.css',import.meta.url),'utf8');
describe('DS-v1.0 contracts',()=>{
 ...
 it('keeps the entry experience out of the legacy editor grid',()=>{expect(novel).toContain('.home.novel-entry{display:block');expect(novel).toContain('width:min(1100px,calc(100% - 32px))')});
});
